...
bool Kystsoft::Sensor::isSupported(sensor_type_e type)
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_is_supported", error);
	return supported;
}
...
bool Kystsoft::Sensor::isWakeUp() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_is_wake_up", error);
	return wakeUp;
}
...
sensor_h Kystsoft::Sensor::defaultSensor(sensor_type_e type)
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_default_sensor", error);
	return sensor;
}
...
std::vector<Kystsoft::Sensor> Kystsoft::Sensor::list(sensor_type_e type /*= SENSOR_ALL*/)
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_sensor_list", error);
...
std::string Kystsoft::Sensor::name() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_name", error);
...
std::string Kystsoft::Sensor::vendor() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_vendor", error);
...
sensor_type_e Kystsoft::Sensor::type() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_type", error);
	return type;
}
...
float Kystsoft::Sensor::minRange() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_min_range", error);
	return range;
}
...
float Kystsoft::Sensor::maxRange() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_max_range", error);
	return range;
}
...
float Kystsoft::Sensor::resolution() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_resolution", error);
	return resolution;
}
...
int Kystsoft::Sensor::minInterval() const
{
	...
	if (error != SENSOR_ERROR_NONE)
		throw FunctionError("sensor_get_min_interval", error);
	return interval;
}
